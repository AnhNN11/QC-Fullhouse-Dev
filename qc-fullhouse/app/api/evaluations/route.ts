...
export async function POST(request: Request) {
  try {
    ...
    const classItem = ObjectId.isValid(String(lesson.classId))
      ? await db.collection("course_classes").findOne({ _id: new ObjectId(String(lesson.classId)) })
      : null;
    if (!classItem) return NextResponse.json({ error: "Buổi học không còn liên kết với lớp hợp lệ." }, { status: 409 });
    ...
    await db.collection("class_sessions").updateOne(
      { _id: lesson._id },
      { $set: { recordingStatus: "reviewed", qcNote: note, updatedAt: now } },
    );
